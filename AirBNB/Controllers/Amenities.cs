...
namespace AirBNB.Controllers
{
    ...
    public class Amenities_campController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Amenities_camp>>> GetAmenities()
        {
            try
            {
                var amenities = _dbContext.amenities.Select(am => new Models.Amenities_only
                {
                    am_id = am.am_id,
                    amenity = am.amenity 
                });
        ...
        [HttpGet("place/{id}")]
        public async Task<ActionResult> GetAmenitiesByPaceId(int id)
        {
            try
            {
                var amen = _dbContext.amenities_camp
                    .Where(am => am.place_id == id)
                    .Join(
                         _dbContext.amenities,  // The other table to join with
                         ac => ac.am_id,        // The foreign key in amenities_camp
                        a => a.am_id,          // The primary key in amenities
                        (ac, a) => new         // The result selector
                        {
                            am_id = a.am_id,
                            amenity_name = a.amenity
                        })
                        .ToList();
                if(amen == null)
                {
